...
final class Trip : Model,ObjectIdentifiable, DataStorage {
    ...
    func getTripDataForUser(user _user: User) throws -> JSON {
        ...
        if isUserTiedToTrip(user: _user) {
            ...
            for data in try getData(level: .guest).enumerated() {
    ...
    static func getUsersTrips(user : User, requester: User) throws -> [Trip] {
        if try user.isUserRelatedTo(user: requester) {
            return try Trip.makeQuery().filter("creator", .equals, user.id!).all()
        }else {
            return try Trip.makeQuery().filter("creator", .equals, user.id!).filter("isPrivate", .equals, false).all()
        }
    }
...
extension Trip: JSONConvertible {
    ...
    func makeJSON() throws -> JSON {
        ...
        try json.set("followers", JSONfollowers)
        try json.set("tripImage", self.attachments)
        try json.set("id",self.id!)
        return json
    }
}
